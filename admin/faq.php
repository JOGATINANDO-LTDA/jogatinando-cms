...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if (!verifyCSRF($_POST['csrf_token'] ?? '')) { flashMessage('error', 'Token inválido.'); header('Location: faq.php'); exit; }
    if ($_POST['action'] === 'save') {
        $question = trim($_POST['question']); $answer = trim($_POST['answer']);
        $sort_order = (int)($_POST['sort_order'] ?? 0); $active = isset($_POST['active']) ? 1 : 0;
        if (empty($question) || empty($answer)) { flashMessage('error', 'Pergunta e resposta são obrigatórias.'); header('Location: faq.php?action=' . ($id > 0 ? "edit&id=$id" : 'new')); exit; }
        if ($id > 0) {
    ...
    if ($_POST['action'] === 'delete') { dbDelete('faq_items', $id); flashMessage('success', 'FAQ excluída.'); header('Location: faq.php'); exit; }
    if ($_POST['action'] === 'toggle') { $r = dbQueryOne("SELECT active FROM faq_items WHERE id = ?", [$id]); if ($r) dbExec("UPDATE faq_items SET active = ? WHERE id = ?", [1 - $r['active'], $id]); header('Location: faq.php'); exit; }
}

if ($action === 'new' || $action === 'edit') {
    $item = $id > 0 ? dbQueryOne("SELECT * FROM faq_items WHERE id = ?", [$id]) : null;
    if ($action === 'edit' && !$item) { flashMessage('error', 'Não encontrada.'); header('Location: faq.php'); exit; }
    ?>
    <div class="card">
